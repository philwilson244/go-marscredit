...
// or whether the Mars Credit app was not started on it.
...
// Mars Credit address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.Mars CreditAddress)
	if _, err := w.trezorExchange(&trezor.Mars CreditGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.Mars CreditSignTx{
	...
	response := new(trezor.Mars CreditTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.Mars CreditTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
