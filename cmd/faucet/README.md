...
First thing's first, the `faucet` needs to connect to an Mars Credit network, for which it needs the necessary genesis and network infos. Each of the following flags must be set:
...
The `faucet` will use the `les` protocol to join the configured Mars Credit network and will store its data in `$HOME/.faucet` (currently not configurable).
