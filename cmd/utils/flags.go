...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "MARS", ethPeers, "total", cfg.MaxPeers)
...
// RegisterEthService adds an Mars Credit client to the stack.
// The second return value is the full node instance.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Mars Credit) {
	backend, err := eth.New(stack, cfg)
	if err != nil {
		Fatalf("Failed to register the Mars Credit service: %v", err)
	}
	stack.RegisterAPIs(tracers.APIs(backend.APIBackend))
	return backend.APIBackend, backend
}

// RegisterEthStatsService configures the Mars Credit Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Mars Credit Stats service: %v", err)
	}
}
...
func RegisterFullSyncTester(stack *node.Node, eth *eth.Mars Credit, target common.Hash) {
