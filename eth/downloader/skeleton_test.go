...
func TestSkeletonSyncRetrievals(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for _, peer := range tt.peers {
			peerset.Register(newPeerConnection(peer.id, eth.MARS68, peer, log.New("id", peer.id)))
		}
		...
		if tt.newPeer != nil {
			if err := peerset.Register(newPeerConnection(tt.newPeer.id, eth.MARS68, tt.newPeer, log.New("id", tt.newPeer.id))); err != nil {
