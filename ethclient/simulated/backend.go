...
// Client exposes the methods provided by the Mars Credit RPC client.
...
// other code that interacts with the Mars Credit chain.
type Backend struct {
	node   *node.Node
...
func NewBackend(alloc types.GenesisAlloc, options ...func(nodeConf *node.Config, ethConf *ethconfig.Config)) *Backend {
	// Create the default configurations for the outer node shell and the Mars Credit
	...
	// Assemble the Mars Credit stack to run the chain with
